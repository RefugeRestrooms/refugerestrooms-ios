...
static NSString *const kRefugeGooglePlacesAutocompleteApiKey =
    @"AIzaSyAs1N-hce2hD16SJyh-QGbpeZIwv5mCSlY";
...
- (BOOL)application:(UIApplication *)application
  ...
  self.appState = [[RefugeAppState alloc]
...
- (NSManagedObjectModel *)managedObjectModel {
  ...
  NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"refuge_ios"
                                            withExtension:@"momd"];
  _managedObjectModel =
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  NSURL *storeURL = [[self applicationDocumentsDirectory]
      URLByAppendingPathComponent:@"refuge_ios.sqlite"];
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath:[storeURL path]]) {
    NSURL *preloadURL =
        [NSURL fileURLWithPath:[[NSBundle mainBundle]
                                   pathForResource:@"RefugeRestroomsSeed"
                                            ofType:@"sqlite"]];
    NSError *error = nil;
    ...
    if (![[NSFileManager defaultManager] copyItemAtURL:preloadURL
      ...
      [[Mixpanel sharedInstance]
          refugeTrackError:error
                    ofType:RefugeMixpanelErrorTypePreloadingRestrooms];
    } else {
...
- (void)setupSearch {
  ...
  [HNKGooglePlacesAutocompleteQuery
      setupSharedQueryWithAPIKey:kRefugeGooglePlacesAutocompleteApiKey
